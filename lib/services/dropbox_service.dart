...
class DropboxService {
  ...
  static Future<String?> uploadFile({
    ...
    if (response.statusCode == 200) {
      ...
      final path = data["path_lower"];

      return await _getSharedLink(path);
    } else {
      return null;
    }
  }

  static Future<String?> _getSharedLink(String path) async {
    final response = await http.post(
      Uri.parse(
    ...
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      return data["url"].replaceAll("?dl=0", "?dl=1");
    }
    return null;
  }
}
